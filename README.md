...
A python library that uses browser automation to login & scrape post details from linkedin
...
```sh
pip install linkedin-post-scraper
```

### Import

```javascript
from linkedin-post-scraper import *
```

### Login

To scrape post details from linkedin first we will login to linkedin. There are two ways of login:-
* Credentials
* Cookies
...
To scrape post details from linkedin we use ``get_post`` function.
It requires **post_link** as input parameter

```javascript
response=linkedin.get_post(post_link="post_link_here")
